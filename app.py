...
section[data-testid="stSidebar"]{background:linear-gradient(180deg,#040d1f 0%,#050f24 100%) !important;border-right:1px solid rgba(0,212,255,0.2) !important;}
section[data-testid="stSidebar"] *{color:#94a3b8 !important;}
section[data-testid="stSidebar"] h2{color:#00d4ff !important;}
.neural-header{background:linear-gradient(135deg,rgba(0,20,50,0.95),rgba(5,15,40,0.95));border:1px solid rgba(0,212,255,0.3);border-radius:20px;padding:36px 44px;margin-bottom:24px;position:relative;overflow:hidden;box-shadow:0 0 40px rgba(0,212,255,0.15);animation:headerFloat 6s ease-in-out infinite;}
@keyframes headerFloat{0%,100%{transform:translateY(0px);}50%{transform:translateY(-6px);}}
.neural-header::after{content:'';position:absolute;top:0;left:-100%;width:60%;height:100%;background:linear-gradient(90deg,transparent,rgba(0,212,255,0.05),transparent);animation:shimmer 4s ease-in-out infinite;}
@keyframes shimmer{0%{left:-100%}100%{left:200%}}
.main-title{font-size:2.8rem;font-weight:900;background:linear-gradient(135deg,#00d4ff 0%,#7b2fff 50%,#00d4ff 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:titleShine 3s linear infinite;margin-bottom:0;line-height:1.2;}
@keyframes titleShine{to{background-position:200% center;}}
.subtitle{color:#64748b;font-size:1rem;margin-top:8px;}
.badge{display:inline-block;background:rgba(0,212,255,0.08);color:#00d4ff;border:1px solid rgba(0,212,255,0.3);border-radius:999px;padding:4px 14px;font-size:0.78rem;font-weight:700;margin:3px;}
.metric-card{background:rgba(0,20,50,0.8);border-radius:16px;padding:20px 24px;border:1px solid rgba(0,212,255,0.2);text-align:center;margin:6px 0;transition:transform 0.35s ease;animation:cardGlow 3s ease-in-out infinite;}
...
div[data-testid="stTabs"] button{background:rgba(0,20,50,0.8) !important;border-radius:10px 10px 0 0 !important;color:#64748b !important;font-weight:600 !important;border:1px solid rgba(0,212,255,0.15) !important;transition:all 0.2s ease !important;}
div[data-testid="stTabs"] button:hover{color:#00d4ff !important;transform:translateY(-2px) !important;}
div[data-testid="stTabs"] button[aria-selected="true"]{background:linear-gradient(135deg,rgba(0,212,255,0.15),rgba(123,47,255,0.15)) !important;color:#00d4ff !important;transform:translateY(-3px) !important;}
div[data-testid="stFileUploader"]{background:rgba(0,20,50,0.6) !important;border:2px dashed rgba(0,212,255,0.3) !important;border-radius:14px !important;transition:all 0.3s !important;}
div[data-testid="stFileUploader"]:hover{border-color:rgba(0,212,255,0.7) !important;}
div[data-testid="stFileUploader"]>div,div[data-testid="stFileUploader"]>div>div,div[data-testid="stFileUploader"] section,div[data-testid="stFileUploader"] section>div{background:rgba(0,15,40,0.9) !important;background-color:rgba(0,15,40,0.9) !important;border-radius:10px !important;border:none !important;}
...
<script>
(function(){const canvas=document.getElementById('neural-canvas');if(!canvas)return;const ctx=canvas.getContext('2d');function resize(){canvas.width=window.innerWidth;canvas.height=window.innerHeight;}resize();window.addEventListener('resize',resize);const nodes=Array.from({length:40},()=>({x:Math.random()*canvas.width,y:Math.random()*canvas.height,vx:(Math.random()-0.5)*0.4,vy:(Math.random()-0.5)*0.4,r:Math.random()*2.5+1.5,pulse:Math.random()*Math.PI*2}));function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);nodes.forEach(n=>{n.x+=n.vx;n.y+=n.vy;n.pulse+=0.02;if(n.x<0||n.x>canvas.width)n.vx*=-1;if(n.y<0||n.y>canvas.height)n.vy*=-1;});nodes.forEach((a,i)=>{nodes.slice(i+1).forEach(b=>{const d=Math.hypot(a.x-b.x,a.y-b.y);if(d<130){const al=Math.floor((1-d/130)*60).toString(16).padStart(2,'0');ctx.beginPath();ctx.strokeStyle='#00d4ff'+al;ctx.lineWidth=0.6;ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}});});nodes.forEach(n=>{const g=Math.sin(n.pulse)*0.5+0.5;ctx.beginPath();ctx.arc(n.x,n.y,n.r+g,0,Math.PI*2);ctx.fillStyle='#00d4ff';ctx.shadowBlur=12+g*8;ctx.shadowColor='#00d4ff';ctx.fill();});requestAnimationFrame(draw);}draw();})();
(function(){const c=document.getElementById('particles-container');if(!c)return;for(let i=0;i<25;i++){const p=document.createElement('div');p.className='particle';p.style.left=Math.random()*100+'vw';p.style.animationDuration=(Math.random()*10+8)+'s';p.style.animationDelay=(Math.random()*10)+'s';p.style.width=p.style.height=(Math.random()*3+1)+'px';p.style.opacity=Math.random()*0.6+0.2;c.appendChild(p);}})();
(function(){const MAX=2.5;let raf=null;document.addEventListener('mousemove',function(e){if(raf)cancelAnimationFrame(raf);raf=requestAnimationFrame(function(){const cx=window.innerWidth/2,cy=window.innerHeight/2;const dx=(e.clientX-cx)/cx,dy=(e.clientY-cy)/cy;const rx=(-dy*MAX).toFixed(2),ry=(dx*MAX).toFixed(2);const c=document.querySelector('.main .block-container');if(c){c.style.transition='transform 0.12s ease-out';c.style.transform=`perspective(1200px) rotateX(${rx}deg) rotateY(${ry}deg)`;}});});document.addEventListener('mouseleave',function(){const c=document.querySelector('.main .block-container');if(c){c.style.transition='transform 0.5s ease';c.style.transform='perspective(1200px) rotateX(0deg) rotateY(0deg)';}});})();
</script>
...
# HELPERS
# ─────────────────────────────────────────────────────────────
def safe_resize(img, max_size=512):
    """Resize image to max_size — prevents OOM on cloud."""
    w, h = img.size
    if max(w, h) > max_size:
...
def set_image(img):
    """Save image to session state and clear old results."""
    img = safe_resize(img)
...
def process_video(video_path, model, original_filename="video.mp4"):
    try:
        ...
        st.error("❌ imageio not found. Add `imageio` and `imageio-ffmpeg` to requirements.txt.")
        return None, {}, 0
    ...
    for frame_rgb in reader:
        pil_f = safe_resize(Image.fromarray(frame_rgb))
        ...
        if frame_idx % 10 == 0:
            preview_ph.image(out_f, caption=f"Frame #{frame_idx} — {nd} object(s)", width="stretch")
    reader.close()
...
def show_video_summary(out_path, all_classes, total_det, total_frames, filename="output.mp4"):
    st.markdown("---"); st.markdown("### 📊 Video Detection Summary")
...
<div class="neural-header">
    ...
    <p class="subtitle">ResNet-50 FPN &nbsp;·&nbsp; MS-COCO 2017 Pretrained &nbsp;·&nbsp; 80 Object Categories</p>
...
with tab1:
    ...
    if uploaded:
        ...
        if ftype.startswith("image"):
            ...
            st.video(uploaded)
            st.markdown('<div class="info-box">⚡ <b>Detection will run on every frame.</b> Download the annotated output video when done.</div>',unsafe_allow_html=True)
        ...
        st.info("ℹ️ Image loaded — click **Run Mask R-CNN Detection** below.")
        st.image(st.session_state['pil_image'],width="stretch")
    ...
    if url:
        ...
        if is_image and not is_video:
            try:
                ...
                st.markdown('<div class="warn-box">💡 Open the URL in browser — it should show only the image, no webpage around it.</div>',unsafe_allow_html=True)
        elif is_video:
            st.markdown(f'<div class="info-box">🎬 <b>Video URL detected!</b></div>',unsafe_allow_html=True)
            if st.button("⬇️ Download & Run Detection",type="primary",width="stretch"):
                ...
                try:
                    with requests.get(url,stream=True,timeout=120,headers={"User-Agent":"Mozilla/5.0"}) as r:
                        ...
                        ct=r.headers.get("Content-Type","").lower()
                        if "text/html" in ct:
                            st.error("❌ This is a webpage. Provide a direct .mp4 URL.")
                            st.stop()
    ...
    if st.button("📥 Load Sample Image",width="stretch"):
        try:
            with st.spinner("Loading..."):
                ...
                img=Image.open(BytesIO(r.content)).convert("RGB")
            set_image(img)
            st.success(f"✅ Loaded! ({st.session_state['pil_image'].size[0]}×{st.session_state['pil_image'].size[1]}px) — Click **Run Mask R-CNN Detection** below!")
            st.image(st.session_state['pil_image'],width="stretch")
        except Exception as e:
            st.error(f"❌ Failed: {e}")
...
# ── Tab 4: Webcam ─────────────────────────────────────────────
with tab4:
    st.markdown('<div class="cloud-box"><h2>📸 Webcam — Photo Mode</h2><p style="color:#64748b;">Click <b>Open Camera</b> to activate. Camera closes automatically after photo is taken.</p></div>',unsafe_allow_html=True)
    col1,col2=st.columns(2)
    open_btn=col1.button("📷 Open Camera",type="primary",width="stretch",disabled=st.session_state['camera_active'])
    close_btn=col2.button("✖️ Close Camera",width="stretch",disabled=not st.session_state['camera_active'])
    if open_btn:
    ...
    if st.session_state['camera_active']:
        ...
        if camera_img is not None:
            ...
            pil_cam=safe_resize(Image.open(camera_img).convert("RGB"))
            with st.spinner("🧠 Loading model..."): model=load_model()
            ...
            st.download_button("⬇️ Download",buf.getvalue(),"webcam_result.png","image/png",width="stretch")
            st.markdown("---"); show_result_details(cam_results,cam_n,score_thr)
    if RUNNING_LOCAL:
        st.markdown("---")
        st.markdown('<div class="local-box"><h3>🎥 Live Video <span style="font-size:0.8rem;color:#64748b;">(Local Only)</span></h3></div>',unsafe_allow_html=True)
        col1,col2=st.columns(2)
        if col1.button("▶️ Start",type="primary",width="stretch"): st.session_state.live_on=True
        if col2.button("⏹️ Stop",width="stretch"): st.session_state.live_on=False
        if st.session_state.live_on:
            st.markdown('<span class="live-badge">● LIVE</span>',unsafe_allow_html=True)
            frame_ph=st.empty(); stats_ph=st.empty()
            with st.spinner("🧠 Loading..."): model=load_model()
            ...
            if not cap.isOpened():
                ...
                while True:
                    ...
                    ret,frame=cap.read()
                    if not ret: break
                    pil_f=safe_resize(Image.fromarray(cv2.cvtColor(frame,cv2.COLOR_BGR2RGB)))
                    res=run_inference(model,pil_f,score_thr)
                    ...
                    frame_ph.image(out_f,caption=f"Frame #{fc} — {nd} object(s)",width="stretch")
...
if pil_image is not None:
    ...
    if st.session_state['result_img'] is not None:
        ...
        with right:
            st.markdown(f"**🎭 Result — {st.session_state['n_det']} instance(s)**")
    ...
    if st.button("🚀 Run Mask R-CNN Detection",type="primary",width="stretch"):
        try:
            with st.spinner("🧠 Loading model..."):
                model=load_model()
            with st.spinner("⚡ Running inference... (may take 30-60s on cloud)"):
            ...
            st.info("💡 Try using a Sample Image from Tab 3, or lower the Confidence Threshold.")
...
with c1:
    with st.expander("ℹ️ What is Mask R-CNN?"):
        st.markdown("""
**Mask R-CNN** is a deep learning model for instance segmentation:
- Detects **80 object categories**
- Draws **bounding boxes** per object
- Generates **pixel-level instance masks**

**Architecture:** ResNet-50 → FPN → RPN → RoI Align → 3 Heads
...
st.markdown("---")
st.markdown('<div class="footer">🎭 Mask R-CNN Instance Segmentation &nbsp;·&nbsp; Built with Streamlit &nbsp;·&nbsp; MS-COCO 2017 &nbsp;·&nbsp; 📁 Image + 🎬 Video + 📷 Webcam</div>',unsafe_allow_html=True)
