...
#  Streamlit UI — All features included
...
section[data-testid="stSidebar"] {
    ...
    box-shadow: 4px 0 24px rgba(0,0,0,0.6), 8px 0 48px rgba(0,0,0,0.3),
...
.neural-header {
    ...
    box-shadow: 0 0 40px rgba(0,212,255,0.15), 0 0 80px rgba(0,100,255,0.08),
...
.metric-card {
    background: rgba(0,20,50,0.8); border-radius: 16px;
    padding: 20px 24px; border: 1px solid rgba(0,212,255,0.2);
...
.metric-card:hover {
    ...
    box-shadow: 0 20px 40px rgba(0,0,0,0.4), 0 0 30px rgba(0,212,255,0.25),
...
.metric-lbl { font-size: 0.78rem; color: #475569; margin-top: 4px; font-weight: 600;
...
div[data-testid="stTabs"] button[aria-selected="true"] {
    ...
    box-shadow: 0 8px 20px rgba(0,0,0,0.45), 0 0 20px rgba(0,212,255,0.25),
...
div[data-testid="stFileUploader"] button {
    background: linear-gradient(135deg, rgba(0,212,255,0.15), rgba(123,47,255,0.15)) !important;
    color: #00d4ff !important; border: 1px solid rgba(0,212,255,0.4) !important;
...
div[data-testid="stExpander"]:hover {
    transform: translateY(-2px) !important; border-color: rgba(0,212,255,0.4) !important;
...
div[data-testid="stButton"] button[kind="primary"] {
    ...
    box-shadow: 0 6px 0 rgba(0,100,150,0.6), 0 8px 16px rgba(0,0,0,0.4),
                0 0 20px rgba(0,212,255,0.4) !important;
    transition: transform 0.1s ease, box-shadow 0.1s ease !important;
}
div[data-testid="stButton"] button[kind="primary"]:hover {
    transform: translateY(-3px) !important;
    box-shadow: 0 9px 0 rgba(0,100,150,0.5), 0 12px 24px rgba(0,0,0,0.5),
                0 0 35px rgba(0,212,255,0.6) !important;
}
div[data-testid="stButton"] button[kind="primary"]:active {
    transform: translateY(4px) !important;
    box-shadow: 0 2px 0 rgba(0,100,150,0.6), 0 2px 6px rgba(0,0,0,0.3),
...
div[data-testid="stSlider"] div[role="slider"] {
    background: #00d4ff !important; box-shadow: 0 0 10px rgba(0,212,255,0.6) !important;
}
...
.warn-box {
    background: rgba(255,170,0,0.06); border-left: 3px solid #ffaa00;
    border-radius: 10px; padding: 14px 18px; color: #94a3b8; margin: 10px 0;
}
.cloud-box {
...
(function() {
    ...
    document.addEventListener('mousemove', function(e) {
        ...
        rafId = requestAnimationFrame(function() {
            ...
            const rotX = (-dy*MAX_TILT).toFixed(2), rotY = (dx*MAX_TILT).toFixed(2);
            const c = document.querySelector('.main .block-container');
            if (c) { c.style.transition='transform 0.12s ease-out';
                     c.style.transform=`perspective(1200px) rotateX(${rotX}deg) rotateY(${rotY}deg)`; }
        });
    });
    document.addEventListener('mouseleave', function() {
        const c = document.querySelector('.main .block-container');
        if (c) { c.style.transition='transform 0.5s ease';
                 c.style.transform='perspective(1200px) rotateX(0deg) rotateY(0deg)'; }
    });
})();
</script>
...
def process_video(video_path, model, original_filename="video.mp4"):
    """Process every frame of a video and return output path + stats."""
    try:
        import imageio
    except ImportError:
        st.error("❌ imageio nahi mila. requirements.txt mein `imageio` aur `imageio-ffmpeg` add karo.")
        return None, {}, 0
    ...
    if total_frames == 0 or width == 0:
        st.error("❌ Video properly open nahi hui — corrupt file ya unsupported format.")
        return None, {}, 0
...
""", unsafe_allow_html=True)
    ...
    try:
        ...
        st.error(f"❌ Video read nahi hui: {e}")
        return None, {}, 0
...
""")
    ...
    with st.spinner("💾 Output video save ho rahi hai..."):
        try:
            import imageio
            writer = imageio.get_writer(
                out_path, format='ffmpeg', fps=fps,
                codec='libx264', output_params=['-pix_fmt','yuv420p']
            )
            for frm in output_frames:
                writer.append_data(frm)
            writer.close()
        except Exception as e:
            st.error(f"❌ Video save nahi hui: {e}")
...
def show_video_summary(out_path, all_classes, total_det, total_frames, filename="output.mp4"):
    """Show summary cards, class table and download button."""
    st.markdown("---")
...
def show_result_details(results, n_det, score_thr):
    ...
    with ch1:
        ...
        for spine in ['bottom','left']: ax.spines[spine].set_color('#1e3a5f')
    ...
    with ch2:
        if n_det > 1:
            ...
            for spine in ['bottom','left']: ax2.spines[spine].set_color('#1e3a5f')
...
with st.sidebar:
    ...
    st.markdown("""<div class="slider-desc">
  <b>Model kitna sure hona chahiye?</b><br>
  ↑ Badhao (0.7+) → Sirf high-confidence objects — accurate<br>
  ↓ Ghataao (0.3-) → Zyada objects — kuch false bhi ho sakte
</div>""", unsafe_allow_html=True)

    mask_thr = st.slider("🎨 Mask Threshold", 0.10, 0.90, 0.50, 0.05)
    st.markdown("""<div class="slider-desc">
  <b>Object ka silhouette kitna tight ho?</b><br>
  ↑ Badhao (0.7+) → Sharp precise edges<br>
  ↓ Ghataao (0.3-) → Loose/bada mask, zyada coverage
</div>""", unsafe_allow_html=True)

    alpha = st.slider("🌫️ Mask Transparency", 0.10, 0.90, 0.45, 0.05)
    st.markdown("""<div class="slider-desc">
  <b>Color overlay kitna dikhega?</b><br>
  ↑ Badhao (0.7+) → Mask zyada solid<br>
  ↓ Ghataao (0.2-) → Original image zyada clear
</div>""", unsafe_allow_html=True)
...
tab1, tab2, tab3, tab4 = st.tabs([
    "📁 Upload File", "🔗 URL", "🖼️ Sample Images", "📷 Webcam"
])
pil_image = None

# ══════════════════════════════════════════════════════════════
# TAB 1 — Upload Image OR Video
# ══════════════════════════════════════════════════════════════
with tab1:
...
⚡ <b>Har frame pe Mask R-CNN detection hogi.</b><br>
Badi video (1min+) pe zyada time lag sakta hai cloud pe.<br>
Processing ke baad <b>annotated output video download</b> kar sakte ho.
            ...
            if st.button("🚀 Start Video Detection", type="primary", use_container_width=True):
                ...
                out_path, all_classes, total_det = process_video(
                    tfile.name, model, uploaded.name
                )
                total_frames = int(cv2.VideoCapture(tfile.name).get(cv2.CAP_PROP_FRAME_COUNT))
...
# ══════════════════════════════════════════════════════════════
# TAB 2 — Image URL or Direct Video URL
# ══════════════════════════════════════════════════════════════
with tab2:
    st.markdown("""<div class="info-box">
💡 <b>Direct file URLs kaam karte hain.</b><br>
Image: <code>.jpg .png .webp</code> &nbsp;|&nbsp; Video: <code>.mp4 .avi .mov .mkv</code><br>
⚠️ YouTube / Bing / Google links <b>supported nahi hain</b> — woh direct file serve nahi karte.
</div>""", unsafe_allow_html=True)

    url = st.text_input(
        "🔗 Direct Image ya Video URL paste karo:",
        placeholder="https://example.com/photo.jpg  ya  https://example.com/video.mp4"
    )

    if url:
        # ── Detect type: pehle Content-Type, phir extension ──
        is_video  = False
        is_image  = False
        ...
        try:
            with st.spinner("🔍 URL check ho raha hai..."):
                head         = requests.head(url, timeout=10, allow_redirects=True,
                                             headers={"User-Agent": "Mozilla/5.0"})
                content_type = head.headers.get("Content-Type", "").lower()

            if any(v in content_type for v in ["video","mpeg","mp4","webm","avi","octet-stream"]):
                is_video = True
            elif any(i in content_type for i in ["image","jpeg","png","gif","webp"]):
                is_image = True
            else:
                # Fallback to extension
                if any(url_clean.endswith(e) for e in video_exts):
                    is_video = True
                elif any(url_clean.endswith(e) for e in image_exts):
                    is_image = True
                else:
                    is_image = True  # Last resort try as image
...
<span style="color:#00d4ff; font-size:0.82rem; word-break:break-all;">{url[:100]}{'...' if len(url)>100 else ''}</span>
            ...
            if st.button("⬇️ Download & Detect Video", type="primary", use_container_width=True):
                ...
                try:
                    ...
                    st.success(f"✅ {downloaded//(1024*1024)} MB downloaded!")

                except Exception as e:
                    st.error(f"❌ Video download nahi hui: {e}")
                    st.markdown("""<div class="warn-box">
💡 <b>Tip:</b> Sirf direct .mp4/.avi links kaam karte hain.<br>
YouTube / Bing / Google Video links supported nahi hain.
</div>""", unsafe_allow_html=True)
                    st.stop()
...
<p style="color:#64748b;">Camera se photo lo — automatic Mask R-CNN detection hoga!</p>
</div>""", unsafe_allow_html=True)
...
Browser mein `http://localhost:8501` → **Webcam → Live Video → ▶️ Start**
        ...
        st.markdown('<div class="info-box">🖥️ <b>Local mode!</b> Dono webcam modes available hain.</div>', unsafe_allow_html=True)
        webcam_mode = st.radio("Mode:", ["📸 Single Photo", "🎥 Live Video"], horizontal=True)
        ...
        if webcam_mode == "📸 Single Photo":
            ...
            if camera_img is not None:
                ...
                with st.spinner("⚡ Detecting..."): cam_results = run_inference(model, pil_cam, score_thr)
                with st.spinner("🎨 Drawing..."):
...
- Objects **detect** karta hai (80 categories)
- Har object ke around **bounding box** draw karta hai
- Pixel-level **instance mask** banata hai

**Pipeline:** ResNet-50 → FPN → RPN → RoI Align → 3 Heads (class + box + mask)
...
<div class="footer">
    ...
    MS-COCO 2017 &nbsp;·&nbsp; 📁 Image + 🎬 Video + 📷 Webcam
</div>
""", unsafe_allow_html=True)
