...
st.set_page_config(page_title="Mask R-CNN", page_icon="🎭", layout="wide")
...
COLORS = [
    ...
    (133,193,233), (240,178,122), (130,224,170), (255,179,102),
...
def visualize_results(pil_image, results, show_masks=True, show_boxes=True, show_labels=True):
    ...
    for i in range(N):
        ...
        if show_masks:
            binary = (masks[i, 0] > 0.5)
            overlay[binary] = (
                overlay[binary] * 0.55 + np.array(color) * 0.45
            ).astype(np.uint8)
        ...
        if show_labels:
            ...
            cv2.putText(overlay, txt, (x1+3, y1-4),
...
def main():
    ...
    score_threshold = st.sidebar.slider("Confidence Threshold", 0.1, 0.9, 0.5, 0.05)
    ...
    input_type = st.radio(
        "Choose input type:",
        ["Upload Image", "Image URL", "Sample Image"]
    )
    ...
    if pil_image is not None:
        ...
        with col2:
            st.subheader(f"Result - {len(results['boxes'])} objects found")
        ...
        col5.metric("Threshold", score_threshold)
...
## Important
```
Make sure your file on GitHub
is named exactly: app.py
